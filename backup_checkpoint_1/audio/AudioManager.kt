...
class AudioManager(private val context: Context) {
    ...
    private fun extractAudioData(uri: Uri): ShortArray? {
        return try {
            ...
            when {
                ...
                uriString.contains(".flac") -> extractAudioDataWithMediaExtractor(uri)
    ...
    private fun extractAudioDataWithMediaExtractor(uri: Uri): ShortArray? {
        return try {
            ...
            while (totalSamples < maxSamples) {
                ...
                // Convert based on format, passing channels for stereo-to-mono conversion
                val samples = convertBytesToShorts(actualData, audioFormat, channels)
                audioData.addAll(samples)
                totalSamples += samples.size
                
                // Debug: log first few samples to see if stereo-to-mono conversion is working
                if (totalSamples < 10) {
                    CrashLogger.log("AudioManager", "First ${samples.size} samples: ${samples.take(5)}")
                }
    ...
    private fun extractAudioDataWithFileStream(uri: Uri): ShortArray? {
        return try {
            ...
            if (riffHeader == "RIFF" && waveHeader == "WAVE") {
                // Parse WAV file with fallback defaults
                var sampleRate = java.nio.ByteBuffer.wrap(header, 24, 4).order(java.nio.ByteOrder.LITTLE_ENDIAN).int
                var channels = java.nio.ByteBuffer.wrap(header, 22, 2).order(java.nio.ByteOrder.LITTLE_ENDIAN).short.toInt()
                var bitsPerSample = java.nio.ByteBuffer.wrap(header, 34, 2).order(java.nio.ByteOrder.LITTLE_ENDIAN).short.toInt()
                
                // Use default values if WAV header is corrupted or invalid
                if (sampleRate <= 0 || sampleRate > 192000) {
                    CrashLogger.log("AudioManager", "Invalid sampleRate=$sampleRate, using default 44100")
                    sampleRate = 44100
                }
                if (channels <= 0 || channels > 8) {
                    CrashLogger.log("AudioManager", "Invalid channels=$channels, using default 2")
                    channels = 2
                }
                if (bitsPerSample <= 0 || bitsPerSample > 32) {
                    CrashLogger.log("AudioManager", "Invalid bitsPerSample=$bitsPerSample, using default 16")
                    bitsPerSample = 16
                }
                ...
                val maxSearchAttempts = 20
                
                while (inputStream.available() > 0 && !dataChunkFound && searchAttempts < maxSearchAttempts) {
                    val chunkHeader = ByteArray(8)
                    val bytesRead = inputStream.read(chunkHeader)
                    if (bytesRead != 8) break
                    
                    val chunkId = String(chunkHeader, 0, 4)
                    val chunkSize = java.nio.ByteBuffer.wrap(chunkHeader, 4, 4).order(java.nio.ByteOrder.LITTLE_ENDIAN).int
                    
                    CrashLogger.log("AudioManager", "Found chunk: $chunkId, size: $chunkSize")
                    
                    if (chunkId == "data") {
                        dataChunkFound = true
                        dataChunkSize = chunkSize
                        CrashLogger.log("AudioManager", "Data chunk found: size=$dataChunkSize")
                        break
                    } else {
                        // Skip this chunk safely
                        val skipSize = if (chunkSize > 0 && chunkSize < 100000000) chunkSize else 0
                        if (skipSize > 0) {
                            inputStream.skip(skipSize.toLong())
                        }
                    }
                    searchAttempts++
                }
                ...
                if (!dataChunkFound) {
                    CrashLogger.log("AudioManager", "Data chunk not found, trying direct read")
                    // Fallback: try to read directly from current position
                }
    ...
    private fun convertBytesToShorts(data: ByteArray, audioFormat: Int, channels: Int = 2): List<Short> {
        val samples = mutableListOf<Short>()
        
        when (audioFormat) {
            android.media.AudioFormat.ENCODING_PCM_16BIT -> {
                val bytesPerFrame = 2 * channels // 2 bytes per sample * channels
                for (i in 0 until data.size step bytesPerFrame) {
                    if (i + 1 < data.size) {
                        val leftSample = ((data[i + 1].toInt() and 0xFF) shl 8) or (data[i].toInt() and 0xFF)
                        if (channels == 2 && i + 3 < data.size) {
                            // Stereo: average left and right channels for mono
                            val rightSample = ((data[i + 3].toInt() and 0xFF) shl 8) or (data[i + 2].toInt() and 0xFF)
                            val monoSample = ((leftSample + rightSample) / 2).toShort()
                            samples.add(monoSample)
                        } else {
                            // Mono or use left channel only
                            samples.add(leftSample.toShort())
                        }
                    }
                }
            }
            android.media.AudioFormat.ENCODING_PCM_8BIT -> {
                for (i in 0 until data.size step channels) {
                    val leftSample = (data[i].toInt() and 0xFF) - 128
                    if (channels == 2 && i + 1 < data.size) {
                        // Stereo: average left and right channels for mono
                        val rightSample = (data[i + 1].toInt() and 0xFF) - 128
                        val monoSample = (leftSample + rightSample) / 2
                        samples.add((monoSample shl 8).toShort())
                    } else {
                        // Mono or use left channel only
                        samples.add((leftSample shl 8).toShort())
                    }
                }
            }
            android.media.AudioFormat.ENCODING_PCM_FLOAT -> {
                val bytesPerFrame = 4 * channels // 4 bytes per sample * channels
                for (i in 0 until data.size step bytesPerFrame) {
                    if (i + 3 < data.size) {
                        val leftFloat = java.nio.ByteBuffer.wrap(data, i, 4).order(java.nio.ByteOrder.LITTLE_ENDIAN).float
                        if (channels == 2 && i + 7 < data.size) {
                            // Stereo: average left and right channels for mono
                            val rightFloat = java.nio.ByteBuffer.wrap(data, i + 4, 4).order(java.nio.ByteOrder.LITTLE_ENDIAN).float
                            val monoFloat = (leftFloat + rightFloat) / 2f
                            val sample = (monoFloat * Short.MAX_VALUE).toInt().coerceIn(Short.MIN_VALUE.toInt(), Short.MAX_VALUE.toInt())
                            samples.add(sample.toShort())
                        } else {
                            // Mono or use left channel only
                            val sample = (leftFloat * Short.MAX_VALUE).toInt().coerceIn(Short.MIN_VALUE.toInt(), Short.MAX_VALUE.toInt())
                            samples.add(sample.toShort())
                        }
                    }
                }
    ...
    private fun generateSpectrogramFromAudioData(audioData: ShortArray): ImageBitmap? {
        return try {
            ...
            // Always convert to mono for consistent spectrogram generation
            // Limit sample size to prevent memory issues and ensure consistent processing
            val maxSamples = 44100 * 30 // 30 seconds at 44.1kHz = 1,323,000 samples
            val limitedAudioData = if (audioData.size > maxSamples) {
                CrashLogger.log("AudioManager", "Limiting audio data from ${audioData.size} to $maxSamples samples")
                audioData.take(maxSamples).toShortArray()
            } else {
                audioData
            }
            
            val monoData = if (limitedAudioData.size % 2 == 0) {
                // Even number of samples - likely stereo, convert to mono
                val mono = ShortArray(limitedAudioData.size / 2)
                for (i in mono.indices) {
                    val left = limitedAudioData[i * 2].toInt()
                    val right = limitedAudioData[i * 2 + 1].toInt()
                    mono[i] = ((left + right) / 2).toShort()
                }
                CrashLogger.log("AudioManager", "Converted stereo to mono: ${limitedAudioData.size} -> ${mono.size} samples")
                mono
            } else {
                // Odd number of samples - likely already mono
                CrashLogger.log("AudioManager", "Using mono audio data: ${limitedAudioData.size} samples")
                limitedAudioData
            }
            
            val width = 1200  // Ultra-high time resolution for maximum detail
            val height = 400  // Ultra-high frequency resolution
            val bitmap = Bitmap.createBitmap(width, height, Bitmap.Config.ARGB_8888)
            
            // Parameters for power spectrogram
            val sampleRate = 44100
            val maxFrequency = 25000  // Limit analysis to 25kHz
            val windowSize = 4096  // Much larger window for ultra-high frequency resolution
            val hopSize = maxOf(1, monoData.size / (width * 4)) // Even smaller hop size for much better time resolution
